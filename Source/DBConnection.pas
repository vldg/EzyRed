...
{$DEFINE FIREDAC} // Not available in Delphi Professional
{$DEFINE DBGO}
{$DEFINE SQLITE}
...
type
  ...
  private
    ...
    {$ENDIF}
    {$IFDEF FIREDAC}
    function CreateFireDACAdapterFromIniFile(AIniFile: TMemIniFile): IDBConnection;
    {$ENDIF}
    {$IFDEF DBGO}
  ...
  public
    ...
    procedure SaveFireDACSettings(AConnectionName: string);
    procedure GetFireDACConnections(AItems: TStrings);
    function IsFireDACSupported: boolean;
...
implementation
uses
  Variants, DB, SysUtils, TypInfo, Dialogs
  {$IFDEF FIREDAC}
  , Main_dmd
  {$ENDIF};
...
//  showmessage(ConnectionFileName);
  ...
  try
    ...
    {$ENDIF}
    {$IFDEF FIREDAC}
    if (AureliusDriver = 'firedac') then
      Result := CreateFireDACAdapterFromIniFile(IniFile);
    {$ENDIF}
    {$IFDEF DBGO}
...
procedure TDBConnection.GetFireDACConnections(AItems: TStrings);
begin
  {$IFDEF FIREDAC}
//  TFireDacConnectionAdapter
//  FDConnection := (IDBConnection as TFireDacConnectionAdapter).Connection;
  {$ENDIF}
end;
...
function TDBConnection.IsFireDACSupported: boolean;
begin
  {$IFDEF FIREDAC}
  Result := true;
  {$ELSE}
...
procedure TDBConnection.SaveFireDACSettings(AConnectionName: string);
var
  IniFile: TMemIniFile;
begin
  IniFile := TMemIniFile.Create(ConnectionFileName);
  try
    IniFile.WriteString('Config', 'AureliusDriver', 'firedac');
    IniFile.WriteString('Config', 'ConnectionName', AConnectionName);
    IniFile.UpdateFile;
  finally
    IniFile.Free;
